...
*/
JNIEXPORT void JNICALL Java_opennlp_ccg_ngrams_SRILMNgramModel_loadLM(JNIEnv *env, jobject obj, jint order, jstring fileName, jint lmType) {

	if(lm != NULL) { // already loaded
		env->ThrowNew(env->FindClass("java/lang/IllegalStateException"),"LM already loaded");
		return;
	}
	nativeLMType = lmType;
	if(nativeLMType < STANDARD || nativeLMType > COUNT) {
		// only STANDARD and COUNT are allowed
		env->ThrowNew(env->FindClass("java/lang/IllegalArgumentException"), "LM type not supported");
		return;
	}
	if(fileName == 0 || env->GetStringLength(fileName) == 0) {
		env->ThrowNew(env->FindClass("java/io/IOException"),"problem reading LM: empty file name");
		return;
	}
	Vocab *vocab = new Vocab;
	lm = (nativeLMType == COUNT) ? (LM *)new NgramCountLM(*vocab, order) : (LM *)new Ngram(*vocab, order);
	const char* nativeFileName = env->GetStringUTFChars(fileName, NULL);
	File file(nativeFileName, "r", 0);
	unsigned lmError = 0;
	if (((File *)&file)->error() > 0) {
		lmError = 1;
		env->ThrowNew(env->FindClass("java/io/IOException"), "problem with LM file");
	}
...
* string.
*/
JNIEXPORT jfloat JNICALL Java_opennlp_ccg_ngrams_SRILMNgramModel_doLogProb(JNIEnv *env, jobject obj, jstring word, jobjectArray context) {
	// make sure LM is ok to use
	if(lm == NULL) {
		env->ThrowNew(env->FindClass("java/lang/IllegalStateException"),"LM not loaded");
		return 0;
	}
	// sanity checks must throw Java exceptions
	if(word == NULL || env->GetStringLength(word) == 0) {
		env->ThrowNew(env->FindClass("java/lang/IllegalArgumentException"),"word is null or zero-length");
		return 0;
	}
	...
	for(unsigned i = 0; i < contextLength; i++) {
		...
		if(jstr == NULL || env->GetStringLength(jstr) == 0) {
			env->ThrowNew(env->FindClass("java/lang/IllegalArgumentException"), "context contains null or zero-length string");
			env->DeleteLocalRef(jstr);
			env->ReleaseStringUTFChars(word, nativeWord); // release word
			// release already translated strings
			for(unsigned j = (i - 1); j >= 0; j--) {
				env->ReleaseStringUTFChars((jstring)env->GetObjectArrayElement(context, j), nativeContext[j]);
			}
			return 0;
		}
...
*/
JNIEXPORT void JNICALL Java_opennlp_ccg_ngrams_SRILMNgramModel_finalize(JNIEnv *env, jobject obj) {
