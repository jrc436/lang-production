package org.jactr.core.model.six.update;
...
public class UpdateCycleProcessor6 extends DefaultCycleProcessor6 {
	/**
	 * logger definition
	 */
	...
	/**
	 * using the current state, guestimate as to the how long this cycle will
	 * run assuming that no production actually fired <br/>
	 * 
	 * @return
	 */
	protected double calculateNextWaitTime(double now, double productionFiringTime, BasicModel model,
		...
		/*
		 * if the production queued any events that should fire immediately, we
		 * need to fire them before we guess what the next time should be.
		 */
		...
		if (!queue.isEmpty())
			nextEventFiringTime = queue.getNextEndTime(); // already
															// constrained

		double nextWaitTime = Math.min(nextProductionFiringTime, nextEventFiringTime);

		/*
		 * no production fired
		 */
		if (Double.isInfinite(productionFiringTime) || Double.isNaN(productionFiringTime)) {
			// if (queue.isEmpty() && !eventsHaveFired)
			// { //commenting this out because
			if (!model.isPersistentExecutionEnabled()) {
				/*
				 * nothing to do, no production fired, and we aren't required to
				 * stay running. lets empty the goal buffer to permit empty
				 * productions (w/ no goal) to fire. if the goal buffer is
				 * already empty, signal quit
				 */
			...
			/*
			 * we only skip cycles if no events have fired. If events have
			 * fired, then productions might be able to fire..
			 */
		...
		if (nextWaitTime <= now) {
			double newWaitTime = Math.nextAfter(now + 0.001, Double.POSITIVE_INFINITY);
			...
			if (LOGGER.isWarnEnabled())
				LOGGER.warn(String.format(
						...
						nextWaitTime, now, newWaitTime, eventsHaveFired, queue.getNextEndTime(), productionFiringTime));

			nextWaitTime = newWaitTime;
		}
		/*
		 * if we didn't fire a production, we might be able to right after the
		 * event has fired
		 */
	 ...
	 * the result.
	 * 
	 * @return firing time or -inf if no production can fire yet, NaN if
	 *         conflict resolution ran but nothing was fired
	 */
	protected double evaluateAndFireProduction(BasicModel model, double currentTime)
		 ...
		 * production could fire. We need the tolerance just in case
		 * nextpossible is 0.1000000002 and current is 0.100000
		 */
		...
		/*
		 * snag the conflict set, procMod should fire the notification events
		 * itself this and selectInstantiation should not be separate methods,
		 * or better yet, provide a third method
		 */
		...
		if (instantiation != null)
			try {
				...
				_nextPossibleProductionFiringTime = BasicClock.constrainPrecision(currentTime + firingDuration);
			} catch (ExecutionException e) {
				throw new ExecutionException("Failed to execute " + instantiation, e.getCause());
			}
		else
			_nextPossibleProductionFiringTime = BasicClock
					.constrainPrecision(currentTime + procMod.getDefaultProductionFiringTime());
