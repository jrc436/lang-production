...
public class FinishConjunctionRight extends SyntaxRule {
	...
	public FinishConjunctionRight(int leftNum, int leftTypeNum, int rightNum, int rightTypeNum, int maxtypes, int maxwords) {
		...
		goalPrecond.add(new Slot(new SSlotName(SSlotNameEnum.LexsynConj, rightNum, maxwords, rightTypeNum, maxtypes), ConjEnum.Conjable));
		...
		effects.add(new Slot(new SSlotName(SSlotNameEnum.LexsynConj, rightNum, maxwords, rightTypeNum, maxtypes), ConjEnum.Nonconjable));
		this.modifyGoal(effects);
		
		this.addOutput(new SlotVar(conjWord).toString()+" was added to the right of "+new SlotVar(fullWord).toString());
	}
}
