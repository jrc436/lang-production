...
public class ConjunctionLeft extends SyntaxRule {
	public ConjunctionLeft(int leftNum, int leftTypeNum, int rightNum, int rightTypeNum, int maxtypes, int maxwords) {
		...
		effects.add(new Slot(new SSlotName(SSlotNameEnum.LexsynConj, rightNum, maxwords, rightTypeNum, maxtypes), ConjEnum.Conjable));
		this.modifyGoal(effects);
		
		this.addOutput(new SlotVar(conjWord).toString()+" was added to the left of "+new SlotVar(fullWord).toString());
	}
