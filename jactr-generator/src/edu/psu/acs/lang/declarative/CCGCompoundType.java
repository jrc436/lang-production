...
public class CCGCompoundType extends CCGType {
	...
	public String toString() {
		return leftCCGType.toString() + connector.toString() + rightCCGType.toString();
	}
	/**
	...
	public CCGOperator getOperator() {
		return connector;
	}
	...
	public static CCGType makeCCGType(String s) {
		//System.out.println("Starting on string:"+s);
		return recCreateTypes(s);
	}
	private static CCGType recCreateTypes(String s) {
		int parenStack = 0;
		int paren = s.indexOf('(');
		int slash = s.indexOf('/');
		...
		if (slash == -1 && bslash == -1) {
			...
			CCGBaseType ct = new CCGBaseType(CCGBaseTypeEnum.value(s), modi);
		...
		CCGCompoundType cg = new CCGCompoundType(recCreateTypes(left), recCreateTypes(right), c);
