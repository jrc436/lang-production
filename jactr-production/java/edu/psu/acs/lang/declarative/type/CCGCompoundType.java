...
public class CCGCompoundType extends CCGType {
	...
	public static CCGType makeCCGType(String s, boolean alternateSeperators) throws ParseException {
		if (s.isEmpty()) {
...
//		 }
	...
	private static CCGType recCreateTypes(String s, char fappSep, char bappSep, ConjEnum conjable) throws ParseException {
		int parenStack = 0;
		int paren = s.indexOf('(');
...
//		}
		if (left.isEmpty() || right.isEmpty()) {
			try {
				if (left.isEmpty()) {
					...
					return recCreateTypes(right, fappSep, bappSep, conjable);
				}
				else {
				...
				System.err.println("Attempt to recover from empty left/right failed.");
			}
	...
	private static ConjEnum checkConjable(String type) throws ParseException {
	...
	private static String stripParens(String s, ConjEnum ce) throws ParseException {
		if (s.isEmpty()) {
			return s;
		}
