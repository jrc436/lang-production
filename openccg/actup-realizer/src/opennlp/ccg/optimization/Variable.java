...
public class Variable {
	...
	public boolean step(boolean goodIncr) throws Exception{
		...
		if (goodIncr) {
			...
			//try the other way, unless it's settled. If it is, settle this way too!
			if (direction < 0.0 && settledP) {
				settledN = true;
				return false; //clearly settled
			}
			else if (direction > 0.0 && settledN) {
				settledP = true;
				return false; //clearly settled
			}
	...
	private boolean checkBounds() throws Exception{
		if (this.currentValue > this.upperBound && direction > 0.0) {
			this.currentValue = this.lastValue;
			settledP = true;
			if (!this.settled()) {
				throw new Exception("it should be settled here...");
			}
			return false;
		}
