...
public class CCGCompoundType extends CCGType {
	...
	public static CCGType makeCCGType(String s, boolean alternateSeperators) {
		...
		if (alternateSeperators) {
			return recCreateTypes(s, new CCGOperator(CCGOperatorEnum.Slash).toString().charAt(0), new CCGOperator(CCGOperatorEnum.Backslash).toString().charAt(0));
		}
	...
	public boolean isConjable() {
		//unfortunately, we need to dig... but that's not that bad!
