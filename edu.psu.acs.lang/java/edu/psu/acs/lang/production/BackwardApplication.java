...
public class BackwardApplication extends SyntaxRule {
	public BackwardApplication(int lexsynLeftNum, int leftTypeNum, int lexsynRightNum, int rightTypeNum, int maxTypes, int maxNumWords) {
		...
		goalPrecond.add(new Slot(new SSlotName(SSlotNameEnum.LexsynString, lexsynRightNum, maxNumWords), new SlotVar(rword)));
		...
		this.addOutput(new SlotVar(leftWord).toString()+" was added to the left of "+new SlotVar(rword).toString());
	}
